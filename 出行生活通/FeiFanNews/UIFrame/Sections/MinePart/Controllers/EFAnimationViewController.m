...
- (void)configViews {
    ...
    NSArray *dataArray = @[@"maps.ico", @"weather", @"collect.ico", @"10", @"12"];
...
//        NSLog(@"自定义处理事件");
//        NSLog(@"%ld,%ld",index,self.currentTag);
        switch (index) {
            case 1001:
                weather.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
                [self presentViewController:weather animated:YES completion:nil];
                break;
            case 1002:
//                collect.modalTransitionStyle = [UIModalTransitionStyleCoverVertical];
//                [self presentViewController:collect animated:YES completion:nil];
                [self.navigationController pushViewController:collect animated:YES];
                break;
            case 1000:
//                maps.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
